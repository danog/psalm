...
abstract class AbstractUnicodeString extends AbstractString
{
    ...
    public function match(string $regexp, int $flags = 0, int $offset = 0): array
    {
        ...
        try {
            $matches = [];
            $result = 'preg_match_all' === $match
                ? preg_match_all($regexp.'u', $this->string, $matches, $flags | \PREG_UNMATCHED_AS_NULL, $offset)
                : preg_match($regexp.'u', $this->string, $matches, $flags | \PREG_UNMATCHED_AS_NULL, $offset);
            if (false === $result) {
