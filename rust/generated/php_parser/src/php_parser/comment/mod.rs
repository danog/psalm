...
impl php_rt::PhpObject for Doc {
    ...
    fn get_prop(&self, name: &str) -> Option<Mixed> { match name { "text" => self.p_text_opt().map(|v| cast::<Mixed>(v)), "startLine" => self.p_startLine_opt().map(|v| cast::<Mixed>(v)), "startFilePos" => self.p_startFilePos_opt().map(|v| cast::<Mixed>(v)), "startTokenPos" => self.p_startTokenPos_opt().map(|v| cast::<Mixed>(v)), "endLine" => self.p_endLine_opt().map(|v| cast::<Mixed>(v)), "endFilePos" => self.p_endFilePos_opt().map(|v| cast::<Mixed>(v)), "endTokenPos" => self.p_endTokenPos_opt().map(|v| cast::<Mixed>(v)), _ => None } }
