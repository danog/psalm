...
impl php_rt::PhpObject for DiffElem {
    ...
    fn get_prop(&self, name: &str) -> Option<Mixed> { match name { "type" => self.p_type__opt().map(|v| cast::<Mixed>(v)), "old" => Some(self.p_old_get()).map(|v| v), "new" => Some(self.p_new__get()).map(|v| v), _ => None } }
...
impl php_rt::PhpObject for Differ {
    ...
    fn get_prop(&self, name: &str) -> Option<Mixed> { match name { "isEqual" => self.p_isEqual_opt().map(|v| Mixed::Closure(Rc::new({ let __c10 = v; DynCallable::new(2, move |__a: Vec<Mixed>| -> Result<Mixed, DynError> { (|| -> Result<Mixed, Throw> { Ok(cast::<Mixed>(__c10(__a[0].clone(), __a[1].clone())?)) })().map_err(|__e| DynError::Obj(cast::<Mixed>(__e))) }) }))), _ => None } }
...
impl php_rt::PhpObject for PrintableNewAnonClassNode {
    ...
    fn get_prop(&self, name: &str) -> Option<Mixed> { match name { "attributes" => self.p_attributes_opt().map(|v| cast::<Mixed>(v)), "attrGroups" => self.p_attrGroups_opt().map(|v| cast::<Mixed>(v)), "flags" => self.p_flags_opt().map(|v| cast::<Mixed>(v)), "args" => self.p_args_opt().map(|v| cast::<Mixed>(v)), "extends" => self.p_extends_opt().map(|v| cast::<Mixed>(v)), "implements" => self.p_implements_opt().map(|v| cast::<Mixed>(v)), "stmts" => self.p_stmts_opt().map(|v| cast::<Mixed>(v)), _ => None } }
...
impl php_rt::PhpObject for TokenStream {
    ...
    fn get_prop(&self, name: &str) -> Option<Mixed> { match name { "tokens" => self.p_tokens_opt().map(|v| cast::<Mixed>(v)), "indentMap" => self.p_indentMap_opt().map(|v| cast::<Mixed>(v)), _ => None } }
...
impl DifferTest {
    ...
    { let _ = self; crate::phpunit::framework::Assert::assertEquals({ let __c1268 = (crate::php_parser::internal::DiffElem::new(crate::php_parser::internal::DiffElem::TYPE_KEEP(), cast::<Mixed>(Str::from_static("a")), cast::<Mixed>(Str::from_static("a")))?, crate::php_parser::internal::DiffElem::new(crate::php_parser::internal::DiffElem::TYPE_KEEP(), cast::<Mixed>(Str::from_static("b")), cast::<Mixed>(Str::from_static("b")))?); let mut __m: Map<ArrayKey, Mixed> = Map::new(); __m.push(cast::<Mixed>(__c1268.0)); __m.push(cast::<Mixed>(__c1268.1)); Mixed::Arr(__m) }, cast::<Mixed>(diff.clone()), Str::from_static(""))? };
    #[allow(unreachable_code)] Ok(())
    }
