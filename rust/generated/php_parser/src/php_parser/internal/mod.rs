...
impl php_rt::PhpObject for DiffElem {
    ...
    fn public_props(&self) -> Vec<(Str, Mixed)> { let mut out = Vec::new(); if let Some(v) = Some(self.p_type__get()) { out.push((Str::from_static("type"), cast::<Mixed>(v))); } if let Some(v) = Some(self.p_old_get()) { out.push((Str::from_static("old"), v)); } if let Some(v) = Some(self.p_new__get()) { out.push((Str::from_static("new"), v)); } out }
...
impl php_rt::PhpObject for Differ {
    ...
    fn call_method(&self, name: &str, args: Vec<Mixed>) -> Result<Mixed, DynError> { match name { "__construct" => { let __r = self.magic__construct((match args.get(0) { Some(__a) => { let __c11 = to_callable(&__a.clone()); Rc::new(move |__p0: Mixed, __p1: Mixed| -> Result<bool, Throw> { Ok(cast::<bool>(__c11.call(vec![__p0, __p1])?)) }) as Rc<dyn Fn(Mixed, Mixed) -> Result<bool, Throw>> }, None => unreachable!("no default for Rc<dyn Fn(Mixed, Mixed) -> Result<bool, Throw>>") })).map_err(|e| DynError::Obj(cast::<Mixed>(e)))?; Ok(__r) }, "diff" => { let __r = self.diff((match args.get(0) { Some(__a) => cast::<Map<ArrayKey, Mixed>>(__a.clone()), None => <Map<ArrayKey, Mixed>>::default() }), (match args.get(1) { Some(__a) => cast::<Map<ArrayKey, Mixed>>(__a.clone()), None => <Map<ArrayKey, Mixed>>::default() })).map_err(|e| DynError::Obj(cast::<Mixed>(e)))?; Ok(cast::<Mixed>(__r)) }, "diffwithreplacements" => { let __r = self.diffWithReplacements((match args.get(0) { Some(__a) => cast::<Map<ArrayKey, Mixed>>(__a.clone()), None => <Map<ArrayKey, Mixed>>::default() }), (match args.get(1) { Some(__a) => cast::<Map<ArrayKey, Mixed>>(__a.clone()), None => <Map<ArrayKey, Mixed>>::default() })).map_err(|e| DynError::Obj(cast::<Mixed>(e)))?; Ok(cast::<Mixed>(__r)) }, "calculatetrace" => { let __r = self.calculateTrace((match args.get(0) { Some(__a) => cast::<Map<ArrayKey, Mixed>>(__a.clone()), None => <Map<ArrayKey, Mixed>>::default() }), (match args.get(1) { Some(__a) => cast::<Map<ArrayKey, Mixed>>(__a.clone()), None => <Map<ArrayKey, Mixed>>::default() })).map_err(|e| DynError::Obj(cast::<Mixed>(e)))?; Ok({ let __c12 = __r; let mut __m: Map<ArrayKey, Mixed> = Map::new(); __m.push(cast::<Mixed>(__c12.0)); __m.push(cast::<Mixed>(__c12.1)); __m.push(cast::<Mixed>(__c12.2)); Mixed::Arr(__m) }) }, "extractdiff" => { let __r = self.extractDiff((match args.get(0) { Some(__a) => cast::<Map<i64, Map<i64, i64>>>(__a.clone()), None => <Map<i64, Map<i64, i64>>>::default() }), (match args.get(1) { Some(__a) => cast::<i64>(__a.clone()), None => <i64>::default() }), (match args.get(2) { Some(__a) => cast::<i64>(__a.clone()), None => <i64>::default() }), (match args.get(3) { Some(__a) => cast::<Map<ArrayKey, Mixed>>(__a.clone()), None => <Map<ArrayKey, Mixed>>::default() }), (match args.get(4) { Some(__a) => cast::<Map<ArrayKey, Mixed>>(__a.clone()), None => <Map<ArrayKey, Mixed>>::default() })).map_err(|e| DynError::Obj(cast::<Mixed>(e)))?; Ok(cast::<Mixed>(__r)) }, "coalescereplacements" => { let __r = self.coalesceReplacements((match args.get(0) { Some(__a) => cast::<Map<ArrayKey, crate::php_parser::internal::DiffElem>>(__a.clone()), None => <Map<ArrayKey, crate::php_parser::internal::DiffElem>>::default() })).map_err(|e| DynError::Obj(cast::<Mixed>(e)))?; Ok(cast::<Mixed>(__r)) }, _ => Err(DynError::Rt(RtError::error(format!("Call to undefined method {}::{}()", "PhpParser\\Internal\\Differ", name)))) } }
}
...
impl DifferTest {
    ...
    { let _ = self; crate::phpunit::framework::Assert::assertEquals({ let __c1251 = (crate::php_parser::internal::DiffElem::new(crate::php_parser::internal::DiffElem::TYPE_KEEP(), cast::<Mixed>(Str::from_static("a")), cast::<Mixed>(Str::from_static("a")))?, crate::php_parser::internal::DiffElem::new(crate::php_parser::internal::DiffElem::TYPE_KEEP(), cast::<Mixed>(Str::from_static("b")), cast::<Mixed>(Str::from_static("b")))?); let mut __m: Map<ArrayKey, Mixed> = Map::new(); __m.push(cast::<Mixed>(__c1251.0)); __m.push(cast::<Mixed>(__c1251.1)); Mixed::Arr(__m) }, cast::<Mixed>(diff.clone()), Str::from_static(""))? };
    #[allow(unreachable_code)] Ok(())
    }
