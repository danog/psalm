...
impl php_rt::PhpObject for FullyQualified {
    ...
    fn get_prop(&self, name: &str) -> Option<Mixed> { match name { "attributes" => self.p_attributes_opt().map(|v| cast::<Mixed>(v)), "name" => self.p_name_opt().map(|v| cast::<Mixed>(v)), "parts" => Some(self.p_parts_get()).map(|v| cast::<Mixed>(v)), _ => None } }
