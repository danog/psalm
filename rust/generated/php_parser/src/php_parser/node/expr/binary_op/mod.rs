...
impl php_rt::PhpObject for BitwiseAnd {
    ...
    fn get_prop(&self, name: &str) -> Option<Mixed> { match name { "attributes" => self.p_attributes_opt().map(|v| cast::<Mixed>(v)), "left" => self.p_left_opt().map(|v| cast::<Mixed>(v)), "right" => self.p_right_opt().map(|v| cast::<Mixed>(v)), _ => None } }
