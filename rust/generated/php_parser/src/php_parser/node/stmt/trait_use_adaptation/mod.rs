...
impl Alias {
    ...
    self.set_p_newName((if (match newName.clone() { Some(__u) => match __u { U_PhpParser_Node_Identifier_or_Str::Str(_) => true, _ => false }, None => false }) { Some(crate::php_parser::node::Identifier::new(cast::<Str>(newName.clone().unwrap()), Map::<Str, Mixed>::new())?) } else { newName.clone().map(|v| cast::<crate::php_parser::node::Identifier>(v)) }));
