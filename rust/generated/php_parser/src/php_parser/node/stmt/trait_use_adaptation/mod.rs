...
pub struct AliasObj {
    ...
    pub newModifier: Option<i64>,
    pub newName: Option<crate::php_parser::node::Identifier>,
}
#[derive(Clone)]
...
impl Alias {
    ...
    pub fn p_newModifier(&self) -> Ref<'_, Option<i64>> { Ref::map(self.0.borrow(), |o| &o.newModifier) }
    pub fn p_newModifier_get(&self) -> Option<i64> { self.0.borrow().newModifier.clone() }
    pub fn p_newModifier_opt(&self) -> Option<Option<i64>> { Some(self.0.borrow().newModifier.clone()) }
    pub fn p_newModifier_mut(&self) -> RefMut<'_, Option<i64>> { RefMut::map(self.0.borrow_mut(), |o| &mut o.newModifier) }
    pub fn set_p_newModifier(&self, v: Option<i64>) { self.0.borrow_mut().newModifier = v; }
    pub fn p_newName(&self) -> Ref<'_, Option<crate::php_parser::node::Identifier>> { Ref::map(self.0.borrow(), |o| &o.newName) }
    pub fn p_newName_get(&self) -> Option<crate::php_parser::node::Identifier> { self.0.borrow().newName.clone() }
    pub fn p_newName_opt(&self) -> Option<Option<crate::php_parser::node::Identifier>> { Some(self.0.borrow().newName.clone()) }
    pub fn p_newName_mut(&self) -> RefMut<'_, Option<crate::php_parser::node::Identifier>> { RefMut::map(self.0.borrow_mut(), |o| &mut o.newName) }
    pub fn set_p_newName(&self, v: Option<crate::php_parser::node::Identifier>) { self.0.borrow_mut().newName = v; }
    pub fn new(mut trait_: Option<crate::php_parser::node::Name>, mut method: U_PhpParser_Node_Identifier_or_Str, mut newModifier: Option<i64>, mut newName: Option<U_PhpParser_Node_Identifier_or_Str>, mut attributes: Map<Str, Mixed>) -> Result<Alias, Throw> {
        let this = Alias(Rc::new(RefCell::new(AliasObj {
            ...
            method: Late::uninit(),
            newModifier: Default::default(),
            newName: Default::default(),
        })));
