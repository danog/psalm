...
php -d memory_limit=5000M psalm -c psalm-transpile-psalm-tests.xml --no-cache --no-progress --transpile-rust=rust/generated/psalm_base --transpile-rust-split=rust/generated/psalm_src:src --transpile-rust-split=rust/generated/psalm_tests:tests > build/transpile-psalm-tests.log 2>&1
...
if ! grep -q "^error" build-psalm.log; then
  timeout 1800 cargo test -j1 -p psalm_tests > test_psalm.log 2>&1
